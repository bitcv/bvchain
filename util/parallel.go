...
type _TaskResultChan <-chan TaskResult

type _TaskResultOk struct {
	TaskResult
	Ok bool
}
...
type TaskResultSet struct {
	chz     []_TaskResultChan
	results []_TaskResultOk
}

func newTaskResultSet(chz []_TaskResultChan) *TaskResultSet {
	return &TaskResultSet{
		chz:     chz,
		results: make([]_TaskResultOk, len(chz)),
	}
}

func (trs *TaskResultSet) Channels() []_TaskResultChan {
	return trs.chz
}
...
func Parallel(tasks ...Task) (trs *TaskResultSet, ok bool) {
	var taskResultChz = make([]_TaskResultChan, len(tasks)) // To return.
		...
		go func(i int, task Task, taskResultCh chan TaskResult) {
			...
			defer func() {
				if pnk := recover(); pnk != nil {
					...
					taskResultCh <- TaskResult{nil, xerr.Trace(pnk, "Panic in task")}
