...
// TrapSignalTerm catches the SIGTERM and executes cb function. 
func TrapSignalTerm(cb func(os.Signal)) {
	Assert(cb != nil)
...
func WriteFileAtomic(filename string, data []byte, perm os.FileMode) error {
	var (
		...
		tempFile = filepath.Join(dir, "write-file-atomic-" + GenerateRandomId(32))
...
func Tempdir(prefix string) (*os.File, string) {
	tempDir := os.TempDir() + "/" + prefix + GenerateRandomId(12)
	err := EnsureDir(tempDir, 0700)
	if err != nil {
		panic(fmt.Sprintf("Error creating temp dir: %v", err))
	}
	dir, err := os.Open(tempDir)
	if err != nil {
		panic(fmt.Sprintf("Error opening temp dir: %v", err))
	}
	return dir, tempDir
}
